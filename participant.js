...
Promise.all([
  ...
  const participant = all.find(r => r.bib === bib);
...
function render(p, details) {
  ...
  document.getElementById('overview-stats').innerHTML = `
    ...
    ${totWorkout?.remark ? `<div class="stat-box"><div class="stat-val">${esc(totWorkout.remark)}</div><div class="stat-lbl">Workouts</div></div>` : ''}
    ${totRun?.remark     ? `<div class="stat-box"><div class="stat-val">${esc(totRun.remark)}</div><div class="stat-lbl">Running</div></div>` : ''}
  ...
  // Classify segments
  const isRun = s => /run|sprint/i.test(s.name) && !/to /i.test(s.name);
  const isTransition = s => /^to /i.test(s.name);
  const isWorkout = s => !isRun(s) && !isTransition(s);
